...
class Program
{
        ...
        while (true)
        {
            ...
            switch (choice)
            {
                ...
                case "5":
                    ...
                    break;
                case "6":
                    await SendDlmsTestMessagesAsync();
                    break;
                case "7":
                    ConfigureSettings();
                    break;
    ...
    private static void PrintMenu()
    {
        ...
        Console.WriteLine("  4. Test připojení (ping)");
        Console.WriteLine("  5. Zátěžový test");
        Console.WriteLine("  6. DLMS testy (profily elektroměrů)  ← NOVÉ");
        Console.WriteLine("  7. Konfigurace");
        Console.WriteLine("  8. Konec (q)");
        Console.WriteLine();
    ...
    {
        Console.WriteLine();
        Console.Write($"Sériové číslo (SN) [náhodné]: ");
        var input = Console.ReadLine();
        var serialNumber =
    ...
    private static async Task SendMultipleMessagesAsync()
    {
        ...
        Console.WriteLine();
        WriteColorLine(
            $"{"#",-4} {"Čas",-15} {"SN",-12} {"Stav",-8} {"Čas (ms)",-10} Odpověď",
            ConsoleColor.Cyan);
        ...
        Console.WriteLine();
        WriteColorLine(
            $"Celkem: {count}, Úspěšných: {successCount}, Neúspěšných: {failCount}",
    ...
    {
        Console.WriteLine();
        WriteColorLine("Zadejte JSON zprávu (prázdný řádek pro odeslání):",
                       ConsoleColor.Yellow);
    ...
    {
        Console.WriteLine();
        Console.Write($"Testuji připojení k {_targetHost}:{_targetPort}... ");
        ...
        try
        {
            ...
            {
                await connectTask;
                WriteColorLine("? Připojení úspěšné!", ConsoleColor.Green);
            }
            else
            {
    ...
    private static async Task SendDlmsTestMessagesAsync()
    {
        Console.WriteLine();
        WriteColorLine("DLMS Testovací zprávy:", ConsoleColor.Cyan);
        WriteColorLine("══════════════════════════════════════════════════════════", ConsoleColor.Gray);

        var allMessages = DlmsTestMessages.GetAllTestMessages();
        for (var i = 0; i < allMessages.Count; i++)
        {
            Console.WriteLine($"  {i + 1,2}. {allMessages[i].Name}");
        }
        Console.WriteLine($"  {allMessages.Count + 1,2}. Odeslat všechny testy");
        Console.WriteLine($"   0. Zpět");
        Console.WriteLine();

        Console.Write("Vyberte test: ");
        var input = Console.ReadLine();

        if (!int.TryParse(input, out var choice) || choice < 0 || choice > allMessages.Count + 1)
        {
            WriteColorLine("Neplatná volba.", ConsoleColor.Yellow);
            return;
        }
        ...
        if (choice == allMessages.Count + 1)
        {
            await SendAllDlmsTestsAsync(allMessages);
        }
        else
        {
            var (name, message) = allMessages[choice - 1];
            WriteColorLine($"\nOdesílám: {name}", ConsoleColor.Yellow);
            await SendAndDisplayResultAsync(message);
        }
    }

    private static async Task SendAllDlmsTestsAsync(IReadOnlyList<(string Name, string Message)> allMessages)
    {
        Console.WriteLine();
        Console.Write("Prodleva mezi zprávami v ms [500]: ");
        var input = Console.ReadLine();
        var delayMs = int.TryParse(input, out var d) ? d : 500;

        Console.WriteLine();
        WriteColorLine($"{"#",-3} {"Test",-30} {"Stav",-8} {"Čas (ms)",-10}", ConsoleColor.Cyan);
        WriteColorLine(new string('─', 60), ConsoleColor.Gray);
        ...
        for (var i = 0; i < allMessages.Count; i++)
        {
            var (name, message) = allMessages[i];
            var result = await SendMessageAsync(message);
            ...
            Console.Write($"{i + 1,-3} {name,-30} ");
            WriteColor($"{status,-8}", statusColor);
            Console.WriteLine($" {result.ElapsedMs,-10}");
        ...
        WriteColorLine(new string('─', 60), ConsoleColor.Gray);
        WriteColorLine(
            $"Celkem: {allMessages.Count}, Úspěšných: {successCount}, Neúspěšných: {failCount}",
            failCount == 0 ? ConsoleColor.Green : ConsoleColor.Yellow);
    }
    ...
    {
        Console.WriteLine();
        Console.Write("Celkový počet zpráv [100]: ");
        var input = Console.ReadLine();
        var messageCount = int.TryParse(input, out var mc) ? mc : 100;

        Console.Write("Počet souběžných připojení [10]: ");
        input = Console.ReadLine();
        var concurrency = int.TryParse(input, out var cc) ? cc : 10;

        Console.WriteLine();
        WriteColorLine(
            $"Spouštím zátěžový test: {messageCount} zpráv, {concurrency} souběžných připojení...",
            ConsoleColor.Yellow);
        Console.WriteLine();
        ...
        WriteColorLine("???????????????????????????????????????",
                       ConsoleColor.Cyan);
        WriteColorLine("  VÝSLEDKY ZÁTĚŽOVÉHO TESTU       ", ConsoleColor.Cyan);
        WriteColorLine("???????????????????????????????????????",
                       ConsoleColor.Cyan);
        Console.WriteLine($"  Celkem zpráv:        {messageCount}");
        WriteColor("  Úspěšných:         ", ConsoleColor.White);
        WriteColorLine($"{results.SuccessCount}", ConsoleColor.Green);
        WriteColor("  Neúspěšných:         ", ConsoleColor.White);
        WriteColorLine($"{results.FailureCount}", ConsoleColor.Red);
        Console.WriteLine(
            $"  Celkový čas:         {stopwatch.ElapsedMilliseconds} ms");
        Console.WriteLine(
            $"  Průměrný čas/zpráva: {(results.SuccessCount > 0 ? results.TotalTime / results.SuccessCount : 0):F2} ms");
        Console.WriteLine(
    ...
    private static async Task SendAndDisplayResultAsync(string json)
    {
        ...
        {
            WriteColorLine(
                $"? Zpráva úspěšně odeslána a potvrzena ({result.ElapsedMs}ms)",
                ConsoleColor.Green);
            if (!string.IsNullOrEmpty(result.Response))
            {
                Console.WriteLine();
                WriteColorLine("Odpověď serveru:", ConsoleColor.Cyan);
                WriteColorLine("?????????????????????????????????????",
                               ConsoleColor.Gray);
